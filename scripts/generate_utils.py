...
def comment_codegen(dst_dir: Union[str, Path], name: str) -> None:
    ...
    for dirpath, _, filenames in os.walk(dst_dir):
        for fn in filenames:
            with open(os.path.join(dirpath, fn), "r", encoding="utf-8") as original:
                file_content = original.read()
            with open(os.path.join(dirpath, fn), "w", encoding="utf-8") as modified:
                modified.write(f"# Code generated by `{name}`. DO NOT EDIT.\n{file_content}")
